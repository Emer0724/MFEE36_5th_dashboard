...
if ($select == 'all') {
    ...
    $page = $page < 1 ? 1 : $page;
    $sql = "SELECT a.*,b.name,c.name as mname,d.status_name FROM used as a JOIN book_info as b using(ISBN) join member as c on a.client_id=c.sid left join book_status as d on a.book_status=d.status_id where deleted is Null order by serial_id desc  ";
    ...
    $page = $page > $totalPage ? $totalPage : $page;
    $page = $page < 1 ? 1 : $page;
    $sql = "SELECT a.*,b.name,c.name as mname ,d.status_name FROM used as a JOIN book_info as b using(ISBN) join member as c on a.client_id=c.sid left join book_status as d on a.book_status=d.status_id where review_status={$select} and deleted is Null order by serial_id desc  ";
    $stmt = $pdo->query($sql)->fetchAll();
}
